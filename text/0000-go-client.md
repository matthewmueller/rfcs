...
```go
results, err := db.Query(fmt.Sprintf(`select * from %s where %s like '%@gmail.com' order by %s + %s desc`, users.Table.Name, users.Column.Email, users.Column.Age, users.Column.PostViewCount))
```
...
```go
bobsPosts, err := posts.FindManyByUserID(db, "bobs-id", posts.First(50))
// or
bobsPosts, err := posts.FindMany(db, posts.Where().UserID('bobs-id'), posts.First(50))
```
